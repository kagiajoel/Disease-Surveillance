...
var url =  '<?php echo base_url()?>'+"weekly_facility/check_facility_data/" + epiweek + "/" + reporting_year + "/" + facility;
$.get(url, function(data) {
if(data == "yes") {
var edit_url = '<?php echo base_url()?>
	'+"weekly_facility/edit_weekly_data/" + epiweek + "/" + reporting_year + "/" + facility;
...
<div class="view_content">
	...
	echo form_open('weekly_facility/save', $attributes);
		...
		<tr>
			...
			<select name="facility" id="facility">
				...
				foreach ($facilities as $facility) {
					if ($facility -> id == $returned_facility) {
						echo '<option selected value="' . $facility -> id . '" district="' . $facility -> District . '" >' . $facility -> Name . '</option>';
					} else {
	...
	?>>
		...
		if ($existing_data == true) {
			$edit_link = base_url()."weekly_facility/edit_weekly_data/".$duplicate_epiweek."/".$duplicate_reporting_year."/". $duplicate_facility[0]->id;
			echo "<p>Epiweek " . $duplicate_epiweek . " data for " . $duplicate_facility[0] -> Name . " district already exists for " . $duplicate_reporting_year . ".</p><p><a class='link' style='margin:0' href='$edit_link'>Click Here</a> to edit the existing data or select different parameters.</p>";
		}
		?>
	</div>
